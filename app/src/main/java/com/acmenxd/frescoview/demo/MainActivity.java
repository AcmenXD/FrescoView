...
import com.acmenxd.frescoview.FrescoUtils;
...
public class MainActivity extends AppCompatActivity {
        ...
        FrescoCallback callback = new FrescoCallback() {
            ...
            public void succeed(String id, ImageInfo imageInfo, Animatable animatable) {
                ...
                if (imageInfo != null) {
                    ...
                    Log.e(TAG, FrescoUtils.appendStrs("Size: ", imageInfo.getWidth(), " x ", imageInfo.getHeight()));
                    Log.e(TAG, FrescoUtils.appendStrs("Quality level: ", qualityInfo.getQuality()));
                    Log.e(TAG, FrescoUtils.appendStrs("good enough: ", qualityInfo.isOfGoodEnoughQuality()));
                    Log.e(TAG, FrescoUtils.appendStrs("full quality: ", qualityInfo.isOfFullQuality()));
                }
                if (animatable != null) {
                    animatable.start();
                }
    ...
    public float dp2px(@FloatRange(from = 0.0D) float dp) {
        return dp2px(this.getResources(), dp);
    }

    public float dp2px(@NonNull Resources resources, @FloatRange(from = 0.0D) float dp) {
